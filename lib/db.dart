...
class DBApp {
  Future createsembast() async {
    final db = await databaseFactoryIo.openDatabase(join('.dart_tool', 'sembast', 'example', 'record_demo.db'));
    // var store = intMapStoreFactory.store('my_store');
    // var key = await store.add(db, {'name': 'ugly'});
    // var record = await (store.record(key).getSnapshot(db) as FutureOr<RecordSnapshot<int, Map<String, Object>>>);
    // record = (await store.find(db, finder: Finder(filter: Filter.byKey(record.key)))).first
    //     as RecordSnapshot<int, Map<String, Object>>;
    // logDebug("createsembast", record.toString());
    ...
    // logDebug("createsembast", records.toString());
  }
}
